...
export const AddressForm = ({ countries, userStoredAddress = {} }: Props) => {
  ...
  const { register, handleSubmit, formState: { isValid }, reset } = useForm<FormInput>({
      defaultValues: {
        ...userStoredAddress,
        rememberAddress: userStoredAddress ? true : false
      }
  }); 

  const { data: session } = useSession({
    required: true // Si esta en true y no esta autenticada lo manda al login
  });
  ...
  useEffect(() => {
    if (address.firstName) {
      reset(address); // Llenar el formulario
    }
  }, [address, reset])
  ...
  const onSubmit = async ( data: FormInput ) => {
    ...
    setAddress(address); // Almacenar en local storage
        ...
        <div className="flex flex-col mb-2 sm:mt-1">
          <div className="inline-flex items-center mb-10">
              <label
                  ...
                  <input
                  type="checkbox"
                  className="border-gray-500 before:content[''] peer relative h-5 w-5 cursor-pointer appearance-none rounded-md border border-blue-gray-200 transition-all before:absolute before:top-2/4 before:left-2/4 before:block before:h-12 before:w-12 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity checked:border-blue-500 checked:bg-blue-500 checked:before:bg-pink-500 hover:before:opacity-10" id="checkbox"  {...register('rememberAddress')}/>
